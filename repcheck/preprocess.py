...
def main():
    ...
    p.add_argument("--output", required=True, help="json file")
    # p.add_argument("--outdir", required=True, help="Output directory for per-run .json files")
    p.add_argument("--start_idx", type=int, default=0)
    p.add_argument("--min_frames", type=int, default=7)
    args = p.parse_args()
    ...
    for r in runs:
        if "data" not in r:
            raise ValueError(f"{args.input} missing key 'data'.")
        ...
        r["data"] = [{"ts": t, "keypoints_3d": s} for t, s in zip(ts, seq_c.tolist())]
        kept += 1
    ...
    print(f"wrote {kept} runs to {args.output}")
