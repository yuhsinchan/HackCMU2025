...
from utils import plot_training_history, plot_multilabel_metrics, print_evaluation_report, count_parameters, set_random_seeds
...
def main():
    ...
    metrics, predictions, targets, probabilities = trainer.evaluate(test_loader)
    
    # Print results
    print_evaluation_report(metrics)
    
    # Plot multi-label metrics
    plot_multilabel_metrics(targets, predictions, probabilities)
    ...
    print(f"\nTraining completed!")
    print(f"Final exact match accuracy: {metrics['exact_match_accuracy']:.4f}")
