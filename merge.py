...
def validate_sample(sample):
    ...
    try:
        ...
        if sample['label'] not in [0, 1, 2,3,4,5,6]:
            print(f"Invalid label: {sample['label']}")
            return False
...
def print_summary(merged_data):
    ...
    correct_samples = sum(1 for sample in merged_data if sample['label'] == 0)
    ...
    print(f"Correct samples (label=0): {correct_samples}")
    print(f"Incorrect samples (labels 1-6): {total_samples - correct_samples}")
    
    print(f"\nLabel distribution:")
    for label in sorted(label_counts.keys()):
        count = label_counts[label]
        percentage = (count / total_samples * 100) if total_samples > 0 else 0
        status = "correct" if label == 0 else "incorrect"
        print(f"  Label {label} ({status}): {count} samples ({percentage:.1f}%)")
