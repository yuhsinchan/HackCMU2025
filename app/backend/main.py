...
parser.add_argument(
    ...
    choices=["zed", "webcam"],
    default="webcam",
    help="Camera mode: zed or webcam",
)
...
async def webcam_feed():
    ...
    try:
        while True:
            ...
            # Prepare frame data
            frame_data = {"type": "frame", "image": frame_b64}
            ...
            if frame_count % 30 == 0:  # Log every 30 frames
                print(
                    f"Sent {frame_count} frames, current active connections: {len(active_connections)}"
                )
        ...
        while True:
            if zed.grab() == sl.ERROR_CODE.SUCCESS:
                ...
                _, buffer = cv2.imencode(
                    ".jpg", image_ocv, [int(cv2.IMWRITE_JPEG_QUALITY), 80]
                )
                # Convert to base64
                frame_b64 = base64.b64encode(buffer).decode("ascii")

                # Broadcast frame to all clients
                await broadcast_frame({"type": "frame", "image": frame_b64})

                # TODO: Implement counter logic based on body tracking data
                global exercise_counter, prediction
                exercise_counter, prediction = (
                    online_processor.get_count_and_prediction()
                )
                await broadcast_counter()
